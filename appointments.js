...
// Add new appointment functionality (keeps the original form submission for new appointments)
...
// Modify existing appointment when Modify button is clicked (real-time)
...
//     const events = appointments.map((appointment, index) => {
//         return {
//             id: index, // Add an ID based on the index to track the specific event
//             title: `${appointment.name} - ${appointment.service}`,
//             start: new Date(`${appointment.date}T${appointment.time}`), // Combine date and time
//             allDay: false // Set to false for specific time slots
//         };
...
//     // Check if this is a new appointment or an existing one (modification)
//     let appointments = JSON.parse(localStorage.getItem('appointments')) || [];
//     if (window.currentEventId !== undefined) {
//         // Update the existing appointment
//         appointments[window.currentEventId] = appointment;

//         // Update the event in the calendar
//         const event = $('#calendar').fullCalendar('clientEvents', window.currentEventId)[0];
//         event.title = `${name} - ${service}`;
//         event.start = new Date(`${date}T${time}`);
//         $('#calendar').fullCalendar('updateEvent', event);
